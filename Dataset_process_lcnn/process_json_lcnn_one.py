...
def process_to_graph_one(data_root, split, dataset):
    dataset_name = data_root.split('/')[-1]

    # for batch in ["training", "validation"]:
    for batch in [split]:
        data_output = os.path.join(data_root, dataset_name + '_lcnn')
        os.makedirs(data_output, exist_ok=True)

        # anno_file = os.path.join(data_root, split, f'{split}+_{sample_index1}+_{sample_index2}+.json')
        # # anno_file = os.path.join(data_root, batch, f"{batch}.json")    # 标注信息
        #
        ...
        def handle(data):
            im = cv2.imread(os.path.join(data_root, batch, "image", data["filename"]))
            print(os.path.join(data_root, "images", data["filename"]))
            ...
            if batch == "training":     # lsun用training， il用train
...
if __name__ == "__main__":
    ...
    dataset_path = '/home/hui/database/dataset/LSUN'
    split = 'validation'
    sample_index1 = 0
    sample_index2 = 10

    anno_path = os.path.join(dataset_path, split, f'{split}+_{sample_index1}+_{sample_index2}+.json')
