...
class Trainer(object):
    def __init__(self, device, model, optimizer, train_loader, val_loader, out):
        ...
        self.out = out  # 模型保存路径
        if not osp.exists(self.out):    # 模型保存路径
        ...
        self.epoch = 0
        self.iteration = 0
        self.max_epoch = C.optim.max_epoch  # 24
        self.lr_decay_epoch = C.optim.lr_decay_epoch
        self.num_stacks = C.model.num_stacks    # 2
        self.mean_loss = self.best_mean_loss = 1e1000   # ?

        self.loss_labels = None
        self.avg_metrics = None
        self.metrics = np.zeros(0)  # 空数组[]

    def run_tensorboard(self):
        board_out = osp.join(self.out, "tensorboard")   # tensorboard文件保存路径
    ...
    def _loss(self, result):
        ...
        '''只对第二个hg module计算lpos和lneg损失
        # [1.2195, 1.3978]中的两个元素表示2个batch
[OrderedDict([('jmap', tensor([1.2195, 1.3978], device='cuda:0', grad_fn=<MulBackward0>)), 
              ('lmap', tensor([0.1738, 0.2001], device='cuda:0', grad_fn=<MulBackward0>)), 
              ('joff', tensor([0.1224, 0.1207], device='cuda:0', grad_fn=<MulBackward0>)), 
              ('lpos', tensor([0.5985, 0.7020], device='cuda:0', grad_fn=<MulBackward0>)), 
              ('lneg', tensor([0.6293, 0.5910], device='cuda:0', grad_fn=<MulBackward0>))]), 
OrderedDict([('jmap', tensor([1.4834, 1.7017], device='cuda:0', grad_fn=<MulBackward0>)), 
             ('lmap', tensor([0.1938, 0.2142], device='cuda:0', grad_fn=<MulBackward0>)), 
             ('joff', tensor([0.1214, 0.1224], device='cuda:0', grad_fn=<MulBackward0>))])]
        '''

        # Don't move loss label to other place.
        # If I want to change the loss, I just need to change this function.
        if self.loss_labels is None:    # 如果是第一个epoch进入字句
            self.loss_labels = ["sum"] + list(losses[0].keys())     # ['sum', 'jmap', 'lmap', 'joff', 'lpos', 'lneg']
            self.metrics = np.zeros([self.num_stacks, len(self.loss_labels)])   # self.num_stacks = 2，这个metric为什么要有两行呢
            print(
                "| ".join(
                    ["progress "]
        ...
        total_loss = 0
        for i in range(self.num_stacks):    # self.num_stacks
            for j, name in enumerate(self.loss_labels):
                ...
                if name not in losses[i]:
                    assert i != 0
                    continue
                loss = losses[i][name].mean()   # 基于某一个类型的损失，对所有batch求平均
                self.metrics[i, 0] += loss.item()
                self.metrics[i, j] += loss.item()
                total_loss += loss  # 所有batch，所有loss的总损失，也就是sum
    ...
    def train_epoch(self):
        ...
        # for batch_idx, sample in enumerate(self.train_loader):
            #   image.shape = (bs, 3, h, w)；meta为列表，内部包含bs个元素，每个元素内部包含了7个数据，见datasets.py；
            #   target为字典，jmap：(bs, 1, 128, 128), joff: (bs, 1, 2, 128, 128), lmap: (bs, 128, 128)

            # input_dict = {
            #     "image": recursive_to(sample['image'], self.device),      # recursive_to：将数据放到gpu上
            #     # "meta": recursive_to(meta, self.device),
            #     "junc": recursive_to(sample["junc"], self.device),
            #     "jtyp": recursive_to(sample["jtyp"], self.device),
            #     "Lpos": recursive_to(sample["Lpos"], self.device),
            #     "Lneg": recursive_to(sample["Lneg"], self.device),
            #     "lpre": recursive_to(sample["lpre"], self.device),
            #     "lpre_label": recursive_to(sample['lpre_label'], self.device),
            #     "lpre_feat": recursive_to(sample['lpre_feat'], self.device),
            #     "target": recursive_to(sample['target'], self.device),
            #
            #     "mode": "training",
            # }
            ...
            loss = self._loss(result)   # 返回的是总loss
    ...
    def _plot_samples(self, i, index, result, meta, target, prefix):
        fn = self.val_loader.dataset.filelist[index][:-10].replace("_a0", "") + ".png"  # 图片名
        img = io.imread(fn) # 图片
        imshow(img), plt.savefig(f"{prefix}_img.jpg"), plt.close()  # 保存原图

        mask_result = result["jmap"][i].cpu().numpy()   # 预测的jmap
        mask_target = target["jmap"][i].cpu().numpy()   # gt的jmap
        for ch, (ia, ib) in enumerate(zip(mask_target, mask_result)):
            imshow(ia), plt.savefig(f"{prefix}_mask_{ch}a.jpg"), plt.close()    # gt交点图
            imshow(ib), plt.savefig(f"{prefix}_mask_{ch}b.jpg"), plt.close()    # 预测的交点图

        line_result = result["lmap"][i].cpu().numpy()   # 预测的lmap
        line_target = target["lmap"][i].cpu().numpy()   # gt lmap
    ...
    def train(self):
        ...
        # if self.iteration == 0:
        #     self.validate()
        epoch_size = len(self.train_loader)     # in train, epoch_size = 20000, in val, epoch_size = 462
        start_epoch = self.iteration // epoch_size
        for self.epoch in range(start_epoch, self.max_epoch):   # self.max_epoch = 24
            if self.epoch == self.lr_decay_epoch:   # 调整学习率
                self.optim.param_groups[0]["lr"] /= 10      # 每过10个epoch，学习率降为原来的1/10
            self.train_epoch()
