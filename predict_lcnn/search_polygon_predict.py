...
def edge_embedding(image_, layout_seg, color):
    ...
    edge_mask = layout_edge > 0.00001

    edge_mask = edge_mask.astype(np.uint8)
    kernel = np.ones((3, 3), np.uint8)
    edge_mask = cv2.dilate(edge_mask, kernel=kernel, iterations=2)
...
def search_ploy(all_points_origin, moved_corner, h, w, layout_seg, img, num_edge_list, threshould, dataset_name):
    '''
    ...
    for NumVertexOfPolygon in num_edge_list:    # 一个多边形由几个顶点组成
    ...
    for m_ in [floor_, ceil_]:
        if np.sum(m_)/((source_h-1)*(source_w-1))>0.05:
            wall_mask_seperate.append(m_)

    pred_seg = np.zeros((source_h, source_w))
    color = 0
    for m_ in wall_mask_seperate:
        color+=1
        pred_seg += m_*color
    # plt.imshow(pred_seg)
    # plt.show()
    ...
    return trans_pred_seg.astype(np.uint8), final_graph
