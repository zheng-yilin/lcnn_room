...
def save_heatmap(prefix, image, lines):
    ...
    lines = lines[:, :, ::-1]   # 将交点顺序改变（xyz改成zyx，或者反过来），有啥用？

    junc = []   # 交点
    jids = {}   # 交点的id，即顺序位置
    # jids通过字典的形式保存了交点坐标和其序号。然后对于一根直线，可以通过具体的坐标表示，也可以通过jids中的交点序号表示
...
def main():
    ...
    # data_root = '/home/hui/database/dataset/wireframe_origin'
    # data_output = '/home/hui/database/dataset/wireframe_origin/gaga'
    data_root = '/home/hui/database/dataset/wireframe_lcnn/wireframe_raw'
    data_output = '/home/hui/database/dataset/wireframe_lcnn/wireframe_raw/gaga'

    os.makedirs(data_output, exist_ok=True)
    for batch in ["train", "valid"]:
    # for batch in ["valid"]:
        anno_file = os.path.join(data_root, f"{batch}.json")    # 标注信息
        ...
        def handle(data):
            ...
            lines = np.array(data["lines"]).reshape(-1, 2, 2)   # (n, 4)-->(n, 2, 2)
            ...
            lines1[:, :, 0] = im.shape[1] - lines1[:, :, 0]     # 图片水平翻转,将每个交点的u坐标替换为[图片宽-每个交点的u坐标]
            lines2 = lines.copy()
            lines2[:, :, 1] = im.shape[0] - lines2[:, :, 1]     # 图片垂直翻转，将每个交点的u坐标替换为[图片高-每个交点的v坐标]
            lines3 = lines.copy()
